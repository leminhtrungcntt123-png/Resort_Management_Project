...
export default function BookingCreateModal({ onClose, onSuccess }: BookingCreateModalProps) {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (!checkInDate || !checkOutDate || !roomId) {
            setError(bForm?.alerts?.fillRequired || "Vui lòng điền đầy đủ thông tin phòng và ngày.");
            return;
        }

        if (isNewCustomer && (!newCustomerName || !newCustomerPhone)) {
            setError(bForm?.alerts?.newCustRequired || "Vui lòng nhập Tên và Số điện thoại cho khách mới.");
            return;
        }

        if (!isNewCustomer && !customerId) {
            setError(bForm?.alerts?.oldCustRequired || "Vui lòng chọn một khách hàng cũ.");
            return;
        }
        ...
        try {
            ...
            setError(errorObj?.response?.data?.message || errorObj?.message || (bForm?.alerts?.errorDefault || "Có lỗi xảy ra khi xử lý"));
        } finally {
            setSaving(false);
        }
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
                <h3 className="text-xl font-bold text-zinc-900 mb-4">
                    {bForm?.createTitle || "Tạo đơn đặt phòng mới"}
                </h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="rounded-lg border border-zinc-200 p-4 bg-zinc-50">
                        <div className="flex items-center justify-between mb-3">
                            <label className="text-sm font-semibold text-zinc-900">
                                {bForm?.customerInfo || "Thông tin khách hàng"}
                            </label>
                            <div className="flex items-center gap-2">
                                ...
                                <label htmlFor="newCustomer" className="text-sm text-blue-600 cursor-pointer font-medium">
                                    {bForm?.newCustomerCheckbox || "+ Thêm khách mới"}
                                </label>
                            </div>
                        </div>
                        ...
                        {isNewCustomer ? (
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <input
                                        type="email"
                                        placeholder={lang === "en" ? "Email (Optional)" : "Email (Tùy chọn)"}
                            ...
                            >
                                <option value="">{bForm?.placeholderSelectCustomer || "-- Tra cứu khách hàng cũ --"}</option>
                                {customers.map((c) => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="mb-1 block text-sm font-medium text-zinc-700">
                                {bForm?.labelCheckIn || "Ngày Check-in *"}
                            </label>
                            <input
                                type="date"
                                min={todayStr}
                        ...
                        <div>
                            <label className="mb-1 block text-sm font-medium text-zinc-700">
                                {bForm?.labelCheckOut || "Ngày Check-out *"}
                            </label>
                            <input
                                type="date"
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-zinc-700">
                            {bForm?.labelSelectRoom || "Chọn Phòng *"}
                        </label>
                        <select
                            value={roomId}
                            ...
                            className="w-full rounded-lg border border-zinc-300 p-2.5 outline-none text-sm"
                        >
                            <option value="">{bForm?.placeholderSelectRoom || "-- Chỉ hiển thị phòng TRỐNG --"}</option>
                            {rooms.map((r) => (
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-zinc-700">
                            {bForm?.labelPaymentMethod || "Phương thức thanh toán"}
                        </label>
                        <select
                            value={paymentMethod}
                            ...
                            className="w-full rounded-lg border border-zinc-300 p-2.5 outline-none text-sm"
                        >
                            <option value="CASH">{bForm?.methods?.CASH || "Tiền mặt (CASH)"}</option>
                            <option value="CARD">{bForm?.methods?.CARD || "Thẻ quẹt (CARD)"}</option>
                            <option value="QR">{bForm?.methods?.QR || "Chuyển khoản / Mã QR (QR)"}</option>
                        </select>
                    </div>
                    ...
                    <div className="mt-6 flex justify-end gap-3 pt-4 border-t border-zinc-100">
                        ...
                        >
                            {saving
                                ? (t?.bookings?.statusExporting || "Đang xử lý...")
                                : (t?.bookings?.btnCreate?.replace("+ ", "") || (lang === "en" ? "Booking" : "Đặt phòng"))}
                        </button>
                    </div>
                </form>
            </div>
