...
export default function BookingDetailModal({ booking, onClose }: Props) {
    ...
    const totalAmount = lang === "en"
        ? `${booking.payment?.amount?.toLocaleString("en-US")} VND`
            ...
            <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-zinc-900">
                        {bForm?.detailTitle || "Chi tiết Booking"} #{booking.id}
                    </h3>
                    <button
                        onClick={onClose}
                ...
                <div className="mt-4 rounded-lg bg-zinc-50 p-4">
                    <p className="text-xs font-medium text-zinc-500 uppercase">
                        {t?.bookings?.table?.customer || "Khách hàng"}
                    </p>
                ...
                <div className="mt-3 grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-zinc-50 p-3">
                        <p className="text-xs text-zinc-500">{t?.bookings?.table?.checkIn || "Check-in"}</p>
                        <p className="font-medium">{booking.checkInDate}</p>
                    </div>
                    <div className="rounded-lg bg-zinc-50 p-3">
                        <p className="text-xs text-zinc-500">{t?.bookings?.table?.checkOut || "Check-out"}</p>
                ...
                <div className="mt-3">
                    <p className="text-xs font-medium text-zinc-500 uppercase">
                        {bForm?.labelRooms || "Phòng"}
                    </p>
                    <div className="mt-1 flex flex-col gap-1">
                        {booking.rooms?.map(room => (
                            <div key={room.roomId}
                                className="flex justify-between rounded-lg border border-zinc-100 px-3 py-2 text-sm">
                                <span>{lang === "en" ? "Room" : "Phòng"} {room.roomNumber} — {room.roomTypeName}</span>
                                <span className="text-zinc-500">
                                    {lang === "en"
                                        ? `${room.priceSnapshot?.toLocaleString("en-US")} VND/${bForm?.labelNight || "night"}`
                                        : `${room.priceSnapshot?.toLocaleString("vi-VN")}đ/${bForm?.labelNight || "đêm"}`}
                                </span>
                            </div>
                        ))}
                    </div>
                ...
                <div className="mt-3 rounded-lg bg-zinc-50 p-4">
                    <p className="text-xs font-medium text-zinc-500 uppercase">
                        {t?.bookings?.table?.payment || "Thanh toán"}
                    </p>
                    <div className="mt-2 flex justify-between">
                        <span className="text-sm">{bForm?.labelTotalAmount || "Tổng tiền"}</span>
                        <span className="font-semibold">{totalAmount}</span>
                    </div>
                    <div className="flex justify-between mt-1">
                        <span className="text-sm">{lang === "en" ? "Method" : "Phương thức"}</span>
                        <span className="text-sm">{booking.payment?.paymentMethod}</span>
                    </div>
                    <div className="flex justify-between mt-1">
                        <span className="text-sm">{bForm?.labelStatus || "Trạng thái"}</span>
