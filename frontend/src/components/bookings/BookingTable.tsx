        ...
        <thead className="bg-zinc-50 text-zinc-500">
          <tr>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.code || "ID"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.customer || "Khách hàng"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.checkIn || "Check-in"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.checkOut || "Check-out"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.status || "Trạng thái"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.payment || "Thanh toán"}
            </th>
            <th className="px-4 py-3 text-left">
              {t?.bookings?.table?.actions || "Thao tác"}
            </th>
          </tr>
        </thead>
                    ...
                    {booking.status === "CHECKED_IN" && (
                      <>
                        ...
                        <button
                          ...
                          className="rounded-lg border border-purple-200 px-2 py-1
                       text-xs text-purple-700 hover:bg-purple-50"
                        >
                          + Dịch vụ
                        </button>
                      </>
