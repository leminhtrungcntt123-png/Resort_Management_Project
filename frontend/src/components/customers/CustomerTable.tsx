        ...
        <div className="mt-6 rounded-xl border border-zinc-200 bg-white overflow-hidden">
            <table className="w-full text-sm">
                <thead className="bg-zinc-50 text-zinc-500">
                    <tr>
                        <th className="px-4 py-3 text-left">{t?.customers?.tableHeaders?.fullName || "Họ tên"}</th>
                        <th className="px-4 py-3 text-left">{t?.customers?.tableHeaders?.phone || "Số điện thoại"}</th>
                        <th className="px-4 py-3 text-left">{t?.customers?.tableHeaders?.email || "Email"}</th>
                        <th className="px-4 py-3 text-left">{t?.customers?.loyaltyPoints || "Điểm tích lũy"}</th>
                        <th className="px-4 py-3 text-left">{t?.customers?.tableHeaders?.actions || "Thao tác"}</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-zinc-100">
                    {loading ? (
                        ...
                        <tr>
                            <td colSpan={5} className="px-4 py-8 text-center text-zinc-400">
                                {t?.customers?.empty || "Không tìm thấy khách hàng nào phù hợp."}
                            </td>
                        </tr>
